...
- `18:21:04`: after narrowing, `blitz_probe` selected rated `300+2` as black against `friendlybot_1700` (`3010`); game `IfUsKs6H` was still active in local log evidence and is not included in aggregate conclusions.
...
- The second post-narrowing accepted sample, `IfUsKs6H`, confirms black-side `300+2` exposure is still reachable; it was deliberately not waited on, so no result-based decision is recorded here.
