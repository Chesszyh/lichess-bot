...
- `18:21:04`: after narrowing, `blitz_probe` selected rated `300+2` as black against `friendlybot_1700` (`3010`); game `IfUsKs6H` ended as a normal black Ruy Lopez Open loss by mate, rating `-4`.
- The refreshed fast aggregate now covers `75` games and has blitz at `-9` rating over `25` scored games, while bullet is `-66` over `32` scored games.
...
- The second post-narrowing accepted sample, `IfUsKs6H`, confirms black-side `300+2` Open Ruy exposure is still leaking.
- Applied an opening-specific mitigation instead of another broad time-control filter: a one-entry black bot polyglot book maps the Ruy position after `5. O-O` to `5...Be7`.
- LaunchAgent `org.chesszyh987.lichess-bot` was safely restarted from PID `33763` to PID `33621` after `/api/account/playing` returned `active_count=0`.
- Next verification target: a black-side Ruy Lopez bot game should log an opening-book `f8e7` move rather than engine-selected `f6e4`.
