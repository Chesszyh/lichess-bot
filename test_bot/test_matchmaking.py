...
def test_challenge__retries_next_candidate_when_opponent_requires_friendship(monkeypatch: MonkeyPatch) -> None:
    """A friend-only opponent endpoint error should not waste the whole matchmaking cycle."""
    mock_li = Mock()
    mock_li.get_online_bots.return_value = [
        {"username": "FriendOnlyBot", "perfs": {"bullet": {"rating": 3010, "games": 100}}},
        {"username": "ReadyBot", "perfs": {"bullet": {"rating": 3005, "games": 100}}},
    ]
    mock_li.get_public_data.side_effect = lambda username: {"username": username}
    mock_li.challenge.side_effect = [
        {"error": "BOT FriendOnlyBot only accepts challenges from friends"},
        {"id": "ready123"},
    ]
    mock_config = Configuration({
        "challenge": {"variants": ["standard"]},
    ...
    assert mock_li.challenge.call_args_list[0].args[0] == "FriendOnlyBot"
    assert mock_li.challenge.call_args_list[1].args[0] == "ReadyBot"
    assert matchmaking.challenge_id == "ready123"
    assert not matchmaking.should_accept_challenge("FriendOnlyBot", "")
    assert matchmaking.challenge_type_acceptable[("FriendOnlyBot", "")].duration == years(10)


def test_challenge__retries_next_candidate_when_opponent_cannot_be_challenged(monkeypatch: MonkeyPatch) -> None:
    """A permanent cannot-challenge endpoint error should not waste the whole matchmaking cycle."""
    mock_li = Mock()
    mock_li.get_online_bots.return_value = [
        {"username": "CannotChallengeBot", "perfs": {"bullet": {"rating": 3010, "games": 100}}},
        {"username": "ReadyBot", "perfs": {"bullet": {"rating": 3005, "games": 100}}},
    ]
    mock_li.get_public_data.side_effect = lambda username: {"username": username}
    mock_li.challenge.side_effect = [
        {"error": "您不能挑战 BOT CannotChallengeBot"},
        {"id": "ready123"},
    ]
    mock_config = Configuration({
        "challenge": {"variants": ["standard"]},
    ...
    assert mock_li.challenge.call_args_list[0].args[0] == "CannotChallengeBot"
    assert mock_li.challenge.call_args_list[1].args[0] == "ReadyBot"
    assert matchmaking.challenge_id == "ready123"
    assert not matchmaking.should_accept_challenge("CannotChallengeBot", "")
    assert matchmaking.challenge_type_acceptable[("CannotChallengeBot", "")].duration == years(10)


def test_challenge__retries_next_candidate_when_challenge_response_has_no_id(monkeypatch: MonkeyPatch) -> None:
    """A target-specific no-id challenge response should not waste the whole matchmaking cycle."""
    mock_li = Mock()
    mock_li.get_online_bots.return_value = [
        {"username": "NoIdBot", "perfs": {"bullet": {"rating": 3010, "games": 100}}},
        {"username": "ReadyBot", "perfs": {"bullet": {"rating": 3005, "games": 100}}},
    ]
    mock_li.get_public_data.side_effect = lambda username: {"username": username}
    mock_li.challenge.side_effect = [
    ...
    assert mock_li.challenge.call_args_list[0].args[0] == "NoIdBot"
    assert mock_li.challenge.call_args_list[1].args[0] == "ReadyBot"
    assert matchmaking.challenge_id == "ready123"
    assert not matchmaking.should_accept_challenge("NoIdBot", "")
