...
class EngineWrapper:
    ...
    def search(self,
        ...
        exact_movetime = (time_limit.time is not None
                          and time_limit.white_clock is None
                          and time_limit.black_clock is None)
        if game and game.speed in ("bullet", "blitz") and exact_movetime and ponder:
    ...
    def record_search_result(self, result: chess.engine.PlayResult, board: chess.Board) -> None:
        ...
        engine_score = result.info.get("score")
        score = engine_score or null_score
...
def apply_bullet_time_management(board: chess.Board, game: model.Game, time_limit: chess.engine.Limit,
    ...
    winning_mate_clock_threshold_ms = bullet_time_management.lookup("winning_mate_clock_threshold_ms")
    winning_mate_clock_ms = bullet_time_management.lookup("winning_mate_clock_ms")
    if (fast_win
            and winning_mate_clock_threshold_ms is not None
    ...
    if to_msec(capped_clock) < clock_ms:
        ...
        force_movetime_threshold_ms = bullet_time_management.lookup("force_movetime_threshold_ms")
        force_movetime = bullet_time_management.lookup("force_movetime_caps")
        if force_movetime and (force_movetime_threshold_ms is None or clock_ms <= force_movetime_threshold_ms):
